...
import static com.cherkashin.telegramBot.constant.Supportive.Commands.CallbackQuery.REQUEST_NEXT_LIST_VACANCIES;

@Component(REQUEST_NEXT_LIST_VACANCIES)
@RequiredArgsConstructor
@FieldDefaults(level = AccessLevel.PRIVATE, makeFinal = true)
public class CallbackQueryRequestТextListVacancies implements CommandRule {
    ...
    public SendMessage execute(Update update, VacancyBot bot, User user) {
        ...
        int countVacancies = vacancyRepository.countAllByActiveTrue();
        int pageSize = env.getProperty("app.max_size_page_vacancies", Integer.class);

        if (pageVacancies * pageSize < countVacancies)
            user.getData().setPageVacancies(++pageVacancies);
